...
app.controller('SidebarCtrl',function($scope, $rootScope, $stateParams, $timeout) {
    ...
    'Non-farmable': '파밍 불가',
    'Global units': '글로벌 전용',
    'Japan exclusives': '일본 전용',
    'In RR pool': '일반뽑기 등장 포함',
    'Not in RR pool': '일반뽑기 등장 제외',
    'Farmable Sockets': '능력소켓 파밍 가능',
    'No Farmable Sockets': '능력소켓 파밍 불가',
    'Inkable': '이펙트 추가 가능',
    ...
    $scope.toggleFilters = function () {
        for (x in $rootScope.filters) {
            if (x.includes("Enabled")) {
                ...
                if (
                    x == "typeEnabled" || x == "characterEnabled" || x == "classEnabled") { $rootScope.filters[x] = !$rootScope.filters["toggle"]; }
                else {

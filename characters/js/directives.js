(function () {
	...
	directives.characterTable = function (
		...
		return {
			...
			link: function (scope, element, attrs) {
				...
				// report link
				var link = $(
					'<span class="help-link"></i><a>문의사항은 원피스트레저크루즈 갤러리에 들어오세요.</a></span>'
				);
				...
				// night toggle
				var nightToggle = $(
					// '<label class="night-toggle"><input type="checkbox">라이트 모드</input></label>' 잠시 주석처리함
				);
				...
				// fuzzy toggle
				var fuzzyToggle = $(
					'<label class="fuzzy-toggle"><input type="checkbox">유사어 검색 허용</input></label>'
				);
				fuzzyToggle.attr(
					"title",
					"활성화 시, 검색어와 정확히 일치하지 않더라도 이름이 유사한 캐릭터들도 함께 표시됩니다.\n특정 캐릭터의 철자를 정확히 모를 때 유용합니다"
				);
	...
	directives.addSupportQuery = function ($state, $stateParams) {
		return {
			...
			scope: { criteria: "=", excludedFamilies: "=" },
			template:
				'<a role="button" ng-if="query" ui-sref="main.search({query:query})"><b>서포트를 장착할 수 있는 캐릭터 검색</b></a>',
			link: function (scope, element, attrs) {
	...
	directives.addSupportersQuery = function () {
		return {
			...
			scope: { uid: "=" },
			template:
				'<a role="button" ng-if="query" ui-sref="main.search({query:query})"><b>장착 가능한 서포트 캐릭터 검색</b></a>',
			link: function (scope, element, attrs) {
		...
		return {
			...
			link: function (scope, element, attrs) {
				...
				if (currentAliases[2]) {
					...
					htmlToAppend +=
						"<tr><td>한국어</td><td><div>" + otherAliases + "</div></td></tr>";
				}
				...
				if (!incomplete && window.flags[id] && window.flags[id].global) {
					...
					ul.append(
						$(
							'<li><a href="' +
								link +
								'" target="_blank">공식가이드 (English)</a></li>'
						)
					);
				}
				if (!incomplete) {
					...
					ul.append(
						$(
							'<li><a href="http://xn--pck6bvfc.gamewith.jp/article/show/' +
								gw[id - 1] +
								'" target="_blank">' +
								'게임위드 (Japanese)</a> | <a href="http://translate.google.com/translate?sl=ja&tl=en&u=http://xn--pck6bvfc.gamewith.jp/article/show/' +
								gw[id - 1] +
								'" target="_blank">' +
								"구글번역</a></li>"
						)
